...
{
    let xhr = new XMLHttpRequest();
    xhr.open("POST","js/ajax.php/KlantagBirtoklasFel",true);
    xhr.send(JSON.stringify({"jarmu" : tank.azon, "legenyseg" : tank.szint, "fullose" : tank.fullos}));
    xhr.addEventListener("readystatechange", function()
    {
        if(this.readyState == 4){
            if(this.status == 200){
...
function osszegyujt()
{
    ...
    for (let i = 1; i < sorokSzama; i++) {
        ...
        tank = new Repulo(sorok[i].id,fullose,szint);
